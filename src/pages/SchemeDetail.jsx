  ...
  if (!scheme) {
    return (
      <div className="container-page py-20 text-center">
        <h1 className="font-display text-2xl font-semibold text-[var(--color-ink)] mb-3">{t("detail_not_found_title")}</h1>
        <p className="text-[var(--color-slate)] mb-6">{t("detail_not_found_desc")}</p>
        <Link to="/schemes" className="text-[var(--color-teal)] font-semibold">{t("detail_back")}</Link>
      </div>
    );
  }
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          {scheme.eligibility?.length > 0 && (
            <section>
              <h2 className="font-display text-lg font-semibold text-[var(--color-ink)] mb-3">{t("detail_who_can_apply")}</h2>
          ...
          {scheme.benefits?.length > 0 && (
            <section>
              <h2 className="font-display text-lg font-semibold text-[var(--color-ink)] mb-3">{t("detail_benefits")}</h2>
              <ul className="space-y-2">
          ...
          {scheme.documents?.length > 0 && (
            <section>
              <h2 className="font-display text-lg font-semibold text-[var(--color-ink)] mb-3">{t("detail_documents")}</h2>
          ...
          {scheme.process?.length > 0 && (
            <section>
              <h2 className="font-display text-lg font-semibold text-[var(--color-ink)] mb-3">{t("detail_process")}</h2>
        ...
        <aside className="space-y-5">
          <div className="bg-white border border-[var(--color-line)] rounded-xl p-6">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-[var(--color-slate)] mb-2">{t("detail_where_to_apply")}</h3>
          ...
          {scheme.notes && (
            <div className="bg-[var(--color-amber-light)] border border-amber-200/60 rounded-xl p-6">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-[var(--color-amber)] mb-2">{t("detail_important_note")}</h3>
